...
class ColorLengthController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		if (Yii::app()->authManager->checkAccess('createColorCode', Yii::app()->user->id)) {
			$model=new ColorLength;
	
			// Uncomment the following line if AJAX validation is needed
			$this->performAjaxValidation($model);
	
			if(isset($_POST['ColorLength']))
			{
				$model->attributes=$_POST['ColorLength'];
				if($model->save())
					$this->redirect(array('view','id'=>$model->color_length));
			}
	...
	{
		if (Yii::app()->authManager->checkAccess('viewColorCode', Yii::app()->user->id)) {
			$dataProvider=new CActiveDataProvider('ColorLength');
	...
	public function actionAdmin()
	{
		if (Yii::app()->authManager->checkAccess('adminColorCode', Yii::app()->user->id)) {
			$model=new ColorLength('search');
			$model->unsetAttributes();  // clear any default values
			if(isset($_GET['ColorLength']))
				$model->attributes=$_GET['ColorLength'];
	 ...
	 * @return ColorLength the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=ColorLength::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param ColorLength $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='color-length-form')
		{
